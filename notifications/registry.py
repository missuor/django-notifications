...
def setup_generic_relations(model_class):
    ...
    if Notification is None:
        raise RegistrationError('Unable get notifications.Notification. Potential circular imports '
                                'in initialisation. Try moving notifications app to come after the '
    ...
    for field in ('actor', 'target', 'action_object'):
        ...
        rel = generic.GenericRelation('notifications.Notification', **kwargs)
        rel.contribute_to_class(model_class, attr)
        relations[field] = rel

        # @@@ I'm not entirely sure why this works
        setattr(Notification, attr_value, None)
    return relations
